...
CREATE TABLE IF NOT EXISTS `tiendaBases`.`Venta` (
  ...
  CONSTRAINT `fk_Venta_Se_Hizo_A_Cliente`
    FOREIGN KEY (`Cliente`)
    REFERENCES `tiendaBases`.`Usuario` (`idCliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `tiendaBases`.`ProductoDeCompra` (
  ...
  `ProductoDelProveedor_IdPDP` VARCHAR(20) NOT NULL,
  PRIMARY KEY (`Compra`, `ProductoDelProveedor_IdPDP`),
  INDEX `fk_Compra_has_Producto_Compra1_idx` (`Compra` ASC) VISIBLE,
  INDEX `fk_ProductoDeCompra_ProductoDelProveedor1_idx` (`ProductoDelProveedor_IdPDP` ASC) VISIBLE,
  CONSTRAINT `fk_Compra_Tiene_Articulos`
    FOREIGN KEY (`Compra`)
    REFERENCES `tiendaBases`.`Compra` (`idCompra`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ProductoDeCompra_ProductoDelProveedor1`
    FOREIGN KEY (`ProductoDelProveedor_IdPDP`)
    REFERENCES `tiendaBases`.`ProductoDelProveedor` (`IdPDP`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
